...
const Header = () => {
    ...
    return (
        <main>
            <nav className="flex fixed top-0 left-0 right-0  z-50 bg-white justify-between px-8 py-6 items-center shadow-lg">
                <div className="flex items-center justify-between gap-10">
                    <section className="flex items-center gap-8">
                        <FiMenu
                            className="3xl cursor-pointer lg:hidden"
                    ...
                    <div className=" flex items-center gap-6 ">
                        {navLinks.map((navItem) => (
                            <Link
                                className="hidden lg:block text-[#757884] hover:text-[#fc8019] font-palanquin font-bold  mx-4  "
                                key={navItem.label}
                                to={`${navItem.link}`}
                            >
                                <div className=" flex items-center gap-3  hover:text-[#fc8019] text-gray-500">
                                    <span className="">{navItem.svg}</span>
                ...
                >
                    <section className="text-black bg-white flex-col absolute left-0 top-0 h-screen p-8 gap-8 z-50 w-56 flex">
                        ...
                        {navLinks.map((navItem) => (
                            <Link
                                to={`${navItem.link}`}
                                className="font-bold text-[#757884] hover:text-[#fc8019] "
                                key={navItem.label}
                            >
                                {navItem.label}
                            </Link>
                ...
                {/* Last Section */}
                <Link to={"/cart"}>
                    <section className="flex items-center gap-2 text-[#757884] hover:text-[#fc8019] font-palanquin font-bold cursor-pointer">
                        <div className="relative">
                            ...
                            <span className="absolute top-[40%] left-1/2 -translate-x-1/2 -translate-y-1/2  ">
                                {cartItems.length}
                            </span>
                        </div>
                        <span>Cart</span>
